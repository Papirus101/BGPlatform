...
from models.bg_request_model import (
    BGRequestCreateSchema,
    BGRequestDetailInfoSchema,
    BGRequestsListSchema,
    BGTypesSpicificsListShema,
)
...
bg_request_router = APIRouter(prefix="/bg_request", tags=["bg_request"])


@bg_request_router.post(
    "/new_bg_request",
    dependencies=[Depends(OAuth2PasswordBearerCookie())],
    response_model=BGRequestCreateSchema,
)
async def create_new_bg_request(
    request: Request,
    bg_request: BGRequestCreateSchema = Body(),
...
async def get_user_request_info(
    request: Request, request_id: int, session: AsyncSession = Depends(get_session)
):
    user = await get_user_by_token(await get_user_token(request), session)
...
@bg_request_router.get(
    ...
    dependencies=[Depends(OAuth2PasswordBearerCookie())],
    status_code=200,
    responses={503: {"service_offline": "zskupki.gov offline"}},
)
...
@bg_request_router.get(
    "/get_valute_types", dependencies=[Depends(OAuth2PasswordBearerCookie())]
)
...
@bg_request_router.get(
    "/get_fz_types", dependencies=[Depends(OAuth2PasswordBearerCookie())]
)
