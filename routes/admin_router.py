...
from db.queries.bg_request_q import get_fz_type_by_name
...
@admin_router.get('/banks_list', dependencies=[Depends(OAuth2PasswordBearerCookie()), Depends(AdminPermission())])
async def get_banks_list(session: AsyncSession = Depends(get_session)):
    banks = await get_all_banks(session)
    return banks


@admin_router.post('/add_bank', dependencies=[Depends(OAuth2PasswordBearerCookie()), Depends(AdminPermission())])
async def add_new_bank(session: AsyncSession = Depends(get_session), bank_info: NewBank = Body()):
