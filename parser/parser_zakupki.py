...
class ZakupkiParse:
    def __init__(self):
        ...
        if settings.DEBUG:
            self.driver = webdriver.Firefox(
                options=options, executable_path="/var/www/api/geckodriver"
            )
        else:
    ...
    async def fz_233_get_data(self, soup, purchase_id):
        self.company_data["purchase_name"] = soup.find(
            "div", {"class": "registry-entry__body-value"}
        ).text.strip()
        lots_link = soup.find_all("a", {"class": "tabsNav__item"})[1].get("href")
        lots_page = await self.__create_request(
            self.domain + lots_link, "lots page", purchase_id
        )
        soup = bss(await lots_page.text(), "html.parser")
        lot_link = (
            soup.find("div", {"id": "inner-html"})
            .find("a", {"target": "_blank"})
            .get("href")
        )
        lot_page = await self.__create_request(
            self.domain + lot_link, "lot page", purchase_id
        )
        soup = bss(await lot_page.text(), "html.parser")
        self.company_data["company_address"] = soup.find(
            "div", {"class": "common-text__value_no-padding"}
        )
        if self.company_data["company_address"] is None:
            self.company_data["company_address"] = ""
        else:
            self.company_data["company_address"] = self.company_data[
                "company_address"
            ].text.strip()

    async def fz_44_get_data(self, soup, purchase_id):
        self.company_data["purchase_name"] = (
            soup.find("div", {"class": "cardMainInfo__section"})
            .find("div", {"class": "cardMainInfo__content"})
            .text.strip()
        )
        self.company_data["company_address"] = (
            soup.find("span", text="Место нахождения")
            .findParent("section", {"class": "blockInfo__section"})
            .find("span", {"class": "section__info"})
            .text.strip()
        )
        organization_link = (
            soup.find("span", text="Размещение осуществляет")
            .findParent("section", {"class": "blockInfo__section"})
            .find("a", {"target": "_blank"})
            .get("href")
        )
        organization_page = await self.__create_request(
            organization_link, "organization page", purchase_id
        )
    ...
    async def test(self, purchase_id: str, request_id: int, session):
        ...
        await self.__create_request(
            self.base_search_url, "base search url", purchase_id
        )
        await asyncio.sleep(5)

        search_page = await self.__create_request(
            self.search_url.format(purchase_id=purchase_id), "search page", purchase_id
        )
        soup = bss(await search_page.text(), "html.parser")
        try:
            href = (
                soup.find("div", {"class": "registry-entry__header-mid__number"})
                .find("a")
                .get("href")
            )
        except AttributeError:
            await send_telegram_error(
                f"🛒 <strong>Закупки</strong> Ошибка при парсинге информации ИД: {purchase_id}"
            )
        ...
        data_page = await self.__create_request(
            self.domain + href, "data page", purchase_id
        )
        ...
        await send_telegram_error(
            f"🛒 <strong>Закупки</strong>Спарсили информацию о компании {self.company_data}"
        )
...
class ZachetniyBiznesParser:
    ...
    async def get_company_name(self, inn: int | str):
        ...
        if data.get("status") != "200":
            await send_telegram_error(
                f"🏪 <strong>Зачётный бизнес</strong> не получилось спарсить имя по ИНН {inn}\n"
                f"{self.search_url}{self.token}&string={str(inn)}"
            )
            await self.session.close()
            return None
    ...
    async def get_info_company_request(self, inn: int | str, request_id: int, session):
        ...
        if company_type_id is None:
            await send_telegram_error(
                f"Ошибка при поиска ид типа компании inn: {inn}, request_id: {request_id}, "
                f'название компании: {self.company_data["company_name"]}'
            )
            return
        self.company_data["company_type_id"] = company_type_id[0].id
        page = await self.session.get(
            f"{self.card_url}{self.token}&id={inn}&_format=json"
        )
        data = await page.json()
        ...
        self.company_data["company_rnp"] = (
            False if data.get("НедобросовПостав") == 0 else True
        )
        self.company_data["company_auth_capital"] = (
            int(data.get("СумКап")) if data.get("СумКап") is not None else 0
        )
        ...
        if (
            data.get("Активность") is not None
            and data.get("Активность") == "В стадии ликвидации"
            or data.get("Активность") == "Ликвидировано"
        ):
            self.company_data["company_bankrupt"] = True
        if data.get(f"ФО{self.last_year}") is not None:
            self.company_data["company_last_revenue_sum"] = data.get(
                f"ФО{self.last_year}"
            ).get("ВЫРУЧКА")
            self.company_data["company_last_revenue_sum"] = (
                0
                if self.company_data["company_last_revenue_sum"] is None
                else int(self.company_data["company_last_revenue_sum"])
            )
        if (
            data.get("СуммНедоимЗадолж") is not None
            and len(data.get("СуммНедоимЗадолж")) > 0
        ):
            for nedoim in data.get("СуммНедоимЗадолж"):
                if isinstance(nedoim, dict):
                    self.company_data["company_tax_arrears_sum"] += int(
                        nedoim["ОбщСумНедоим"].split(",")[0].replace(" ", "")
                    )
                else:
                    self.company_data["company_tax_arrears_sum"] += nedoim
        if (
            data.get("СудыСтатистика") is not None
            and len(data.get("СудыСтатистика")) > 0
        ):
            for otvet in data.get("СудыСтатистика").keys():
                if otvet == "Ответчик":
                    self.company_data["company_arbitration_cases_sum"] += (
                        data.get("СудыСтатистика").get(otvet).get("Сумма")
                    )
                ...
                if fssp_data is not None:
                    ...
                    if fssp_data is not None and isinstance(fssp_data, dict):
                        for executed_list in fssp_data:
                            self.company_data[
                                "company_executed_lists_sum"
                            ] += executed_list.get("СуммаДолга")
        ...
        if self.company_data["company_mass_address"] is not None:
            ...
            if search_address.status == 200:
                ...
                if (
                    mass_address_data is not None
                    and len(mass_address_data.get("docs")) > 10
                ):
        ...
        if resident_page.status == 200:
            ...
            if len(resident_data.get("body")) > 0:
                for datas in resident_data.get("body"):
                    if (
                        datas.get("I_SUBJECT").get("@attributes").get("IS_RESIDENT")
                        is not None
                    ):
                        self.company_data["company_resident"] = (
                            datas.get("@attributes").get("IS_RESIDENT")
                            if datas.get("@attributes").get("IS_RESIDENT") is not None
                            else True
                        )
                        break
        await self.session.close()
        await send_telegram_error(
            f"🏪 <strong>Зачётный бизнес</strong> спарсили информацию {self.company_data}"
        )
        await update_request_info(session, request_id, **self.company_data)
