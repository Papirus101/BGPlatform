...
class BankCompanyTypes(Base):
    ...
    company_type_id = Column(
        ForeignKey("company_types.id", ondelete="CASCADE"), nullable=False
    )
...
class Banks(Base):
    ...
    fz_types = relationship(
        "FZTypes", secondary=BankFzTypes.__tablename__, backref="fz_types_bank"
    )
    bg_types = relationship(
        "BGTypes", secondary=BGTypesBank.__tablename__, backref="bg_types_bank"
    )
    ...
    company_type = relationship(
        "CompanyTypes",
        secondary=BankCompanyTypes.__tablename__,
        backref="comany_types_bank",
    )
    ...
    valute_types = relationship(
        "ValuteTypes",
        secondary=BankValuteTypes.__tablename__,
        backref="bank_valute_type",
    )  # отдельная логика
    ...
    work_specific = relationship(
        "WorksSpecifics", secondary=WSTypesBank.__tablename__, backref="ws_banks"
    )
