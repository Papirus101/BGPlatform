...
QUARTER_CHOICE = [("profit", "прибыль"), ("lesion", "убыток")]
...
class BGRequest(Base):
    ...
    days = Column(Integer)
    bg_type_id = Column(
        ForeignKey(BGTypes.id, ondelete="CASCADE"), nullable=False, default=1
    )
    last_quarter = Column(ChoiceType(QUARTER_CHOICE))
    lesion_amount = Column(Integer, default=0)
    specifics_of_work_id = Column(
        ForeignKey(WorksSpecifics.id, ondelete="CASCADE"), nullable=False, default=1
    )
    ...
    company_fz_id = Column(
        Integer, ForeignKey("fz_types.id", ondelete="CASCADE"), nullable=True
    )

    company_type_id = Column(
        ForeignKey(CompanyTypes.id, ondelete="CASCADE"), nullable=True
    )
    ...
    banks_ids = relationship(
        "Banks", secondary=BanksRequest.__tablename__, backref="banks_request"
    )
...
def insert_test_datas_work_specifics(mapper, connection, *args, **kwargs):
    ...
    connection.execute(
        specific.insert().values(
            name="тестовая спецификация", description="тест описания"
        )
    )
