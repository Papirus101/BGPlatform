...
async def get_user_requests_query(session, user_id: int):
    ...
    sql = (
        select(
            BGRequest.id,
            BGRequest.purchase_number,
            BGRequest.amount,
            BGRequest.days,
            BGRequest.company_name,
            BGRequest.is_ready,
        )
        .where(BGRequest.user_id == user_id)
        .order_by(BGRequest.id)
    )
    data = await session.execute(sql)
    data = data.all()
    return data
...
async def get_banks_with_terms(session, request):
    ...
    sql = banks_request_sql.format(
        ...
        request_company_type=request.company_types.id
        if request.company_types is not None
        else 0,
...
async def get_company_type_by_name(session, company_type: str):
    sql = select(CompanyTypes).where(
        func.lower(CompanyTypes.name).like(company_type.strip().lower())
    )
    try:
        data = await session.execute(sql)
        data = data.one()
...
async def get_bg_type_by_name(session, bg_type_name: str):
    sql = select(BGTypes).where(
        func.lower(BGTypes.name).like(bg_type_name.strip().lower())
    )
    try:
        data = await session.execute(sql)
        data = data.one()
